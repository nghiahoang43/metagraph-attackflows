...
class Metagraph(object):
    ...
    def get_all_metapaths_from(self, source, target):
        ...
        for tmp_outvertex in tmp_outvertexs:
            if Edge(source, tmp_outvertex) not in tmp_edge_list and Edge(source, tmp_outvertex) not in self.edges:
    ...
    def get_degree_for_node(self, node):
        if node not in self.generating_set:
            return 0
        currentDegree = 0
        for edge in self.edges:
            if node in edge.outvertex:
                currentDegree+=1
        return currentDegree
    
    def get_rank_for_node(self, node):
        if node not in self.generating_set:
            return 0
        currentRank = 0
        for edge in self.edges:
            if node in edge.invertex:
                currentRank+=1
        return currentRank

    def get_most_degree_nodes(self):
        degreeList = []
        for node in self.generating_set:
            degreeList.append([self.get_degree_for_node(node), node])
        degreeList.sort(reverse=True)
        highest_degree = degreeList[0][0]
        result = []
        i = 0
        while i < len(degreeList) and degreeList[i][0] == highest_degree:
            result.append(degreeList[i][1])
            i+=1
        result.sort()
        return result
    
    def get_most_rank_nodes(self):
        rankList = []
        for node in self.generating_set:
            rankList.append([self.get_degree_for_node(node), node])
        rankList.sort(reverse=True)
        highest_rank = rankList[0][0]
        result = []
        i = 0
        while i < len(rankList) and rankList[i][0] == highest_rank:
            result.append(rankList[i][1])
            i+=1
        result.sort()
        return result
